...
<section class="card">
    ...
    @forelse ($invoices as $invoice)
        <tr><td dir="ltr"><bdi>{{ $invoice->invoice_no }}</bdi></td><td>{{ $invoice->patient->fullName() }}</td><td dir="ltr"><bdi>{{ $invoice->issue_date ? \App\Support\JalaliDate::format($invoice->issue_date) : '—' }}</bdi></td><td><span class="status-badge status-badge--info">{{ ['issued' => 'صادرشده', 'partially_paid' => 'پرداخت ناقص', 'paid' => 'تسویه‌شده'][$invoice->status] ?? $invoice->status }}</span></td><td dir="ltr"><bdi>{{ number_format((float) $invoice->total, 2) }}</bdi></td><td dir="ltr"><bdi>{{ number_format((float) $invoice->paid_total, 2) }}</bdi></td><td dir="ltr"><bdi>{{ $invoice->balance() }}</bdi></td><td><a class="button button--ghost button--small" href="{{ route('invoices.show', ['invoiceId' => $invoice->id]) }}">مشاهده</a></td></tr>
    @empty
